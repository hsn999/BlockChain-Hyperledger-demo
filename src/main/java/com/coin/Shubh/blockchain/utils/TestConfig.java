 ...
 */
package com.coin.Shubh.blockchain.utils;
...
public class TestConfig {
    ...
    private final static HashMap<String, Organisation> sampleOrgs = new HashMap<>();
...
//                    DEFAULT_CONFIG));
            ...
            for (Map.Entry<Object, Object> x : sdkProperties.entrySet()) {
                ...
                if (key.startsWith(INTEGRATIONTESTS_ORG)) {
                    ...
                    if (match.matches() && match.groupCount() == 1) {
                        ...
                        sampleOrgs.put(orgName, new Organisation(orgName, val.trim()));

                    }
                }
            }

            for (Map.Entry<String, Organisation> org : sampleOrgs.entrySet()) {
                final Organisation sampleOrg = org.getValue();
                ...
                if (runningFabricCATLS) {
                    String cert = "fixture/sdkintegration/e2e-2Orgs/channel/crypto-config/peerOrganizations/DNAME/ca/ca.DNAME-cert.pem".replaceAll("DNAME", domainName);
    ...
    public Collection<Organisation> getIntegrationTestsSampleOrgs() {
        return Collections.unmodifiableCollection(sampleOrgs.values());
    }

    public Organisation getIntegrationTestsSampleOrg(String name) {
